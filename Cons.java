/*s class Cons implements a Lisp-like Cons cell
 * 
     ...
     // 2. letter count, case-insensitive
        ...
        final Mapper map2 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	for(int i=0; i<length;i++){
            		...
            		int unicode=Character.getNumericValue(current);
            		//System.out.println(current + " " +unicode);
            		if (unicode >=10) {
            			mr.collect_map(Character.toString(current), list("1"));
					}	
            	}
            } 
         };
        ...
        final Reducer red2 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	int sendThis=0;
            	 for ( Cons lst = value; lst != null; lst = rest(lst) ){
            		sendThis+=  Integer.decode((String) first((Cons) first(lst)));	
            	}
            	mr.collect_reduce(key, new Integer(sendThis));
            } 
        };
     ...
     // 3. word count, case-insensitive
        ...
        final Mapper map3 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	String lowerCase=value.toLowerCase();
            	String [] all=lowerCase.split("[^a-z]"); //this took forever to figure out
            	for(String current : all){
            		if(!"".equals(current)){	
            			mr.collect_map((String)current, list("1"));
            		}
            	}
            } 
        };
        ...
        final Mapper map4 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                // System.out.println("   grepkey = " + grepkey); wat
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	Cons sendThis=null;
            	for (Cons lst = value; lst != null; lst = rest(lst)) {
					sendThis = cons( Integer.decode((String) first((Cons) first(lst))), sendThis);
				}
				sendThis = nreverse(sendThis);
				mr.collect_reduce(key, sendThis);
            } 
        };
        ...
        // 5. index, case-insensitive  basically 3map 4reduce
        
        final Mapper map5 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                // System.out.println("   map5 key = " + key + "  val = " + value);
            	String lowerCase=value.toLowerCase();
            	String [] all=lowerCase.split("[^a-z]"); 
            	for(String current : all){
            		if(!"".equals(current)){	
            			mr.collect_map((String)current, list(key));
            		}
            	}
            } 
       };
     ...
     // 6. movie reviews
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	//System.out.println(info[0] + " " + info[2]);
        ...
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
				...
				mr.collect_reduce(key, list((double)sendThis/total , total) );
            } 
        };
